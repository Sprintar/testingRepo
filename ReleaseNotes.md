...
- **Import API Endpoint with Preview - BE** - Added the ability to import Assistants into the application.
- **Abstract Document Parser Module** - Introduced the `DOCUMENT_PARSER` module type and the `AbstractDocumentParser` base class to standardize document parsing interfaces. Added the `DocumentElement` structure, which ensures natural reading order and full typing.
