...
## Features Released:
- **Enhance Structured Data Capability** - Added support for drag-and-drop functionality to upload structured data (CSV/Excel) into the message component for in-memory processing when asking questions. Attachments are also supported. Limitations include limited support for complex Excel files (e.g., multiple tables, macros) and datasets uploaded via the message component, which are still processed using the RAG approach.
...
- **[P2] SPIKE: Release Branch Controls** - Documented the current release process and proposed a new versioning process. Key changes include tagging Docker builds with version numbers, replacing the `release` branch with tags, and introducing clear versioning patterns for major, minor, and service pack releases.
